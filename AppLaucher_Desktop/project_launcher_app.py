#!/usr/bin/env python3
"""
Project Launcher - Aplicativo Desktop
Janela nativa Windows com WebView embutido.

Para gerar o .exe, rode: build.bat
"""
...
# ── Caminho dos dados (funciona tanto em .py quanto em .exe) ──────────────────
...
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  /* Titlebar customizada para arrastar a janela */
  .titlebar {
  ...
  .logo-icon {
    ...
    font-size: 20px; box-shadow: 0 0 20px rgba(110,231,183,0.3);
  }
  ...
  .btn-new {
    background: linear-gradient(135deg, var(--accent), var(--accent2));
    color: #0a0a0f; border: none; padding: 10px 20px; border-radius: 8px;
  ...
  #search {
    ...
    color: var(--text); padding: 12px 14px 12px 42px; border-radius: var(--radius);
  ...
  .card-color { width: 10px; height: 10px; border-radius: 50%; flex-shrink: 0; box-shadow: 0 0 8px currentColor; }
  ...
  .badge { font-family: 'DM Mono', monospace; font-size: 10px; padding: 3px 8px; border-radius: 20px; background: var(--surface2); border: 1px solid var(--border); color: var(--muted); white-space: nowrap; }

  .btn-launch {
    background: var(--accent); color: #0a0a0f; border: none;
    padding: 7px 14px; border-radius: 6px; font-family: 'Syne', sans-serif;
    font-weight: 700; font-size: 11px; cursor: pointer; letter-spacing: 0.5px; transition: all 0.2s; white-space: nowrap;
  }
  .btn-launch:hover { background: #a7f3d0; box-shadow: 0 4px 12px rgba(110,231,183,0.3); }

  .card-actions { display: flex; gap: 6px; }
  .btn-icon { background: transparent; border: 1px solid var(--border); color: var(--muted); width: 30px; height: 30px; border-radius: 6px; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: all 0.2s; font-size: 13px; }
  ...
  .items-panel { border-top: 1px solid var(--border); padding: 0 20px; max-height: 0; overflow: hidden; transition: max-height 0.3s ease, padding 0.3s ease; }
  .project-card.open .items-panel { max-height: 600px; padding: 16px 20px; }

  .item-row { display: flex; align-items: center; gap: 10px; padding: 8px 10px; border-radius: 8px; transition: background 0.15s; margin-bottom: 4px; }
  .item-row:hover { background: var(--surface2); }
  .item-type-icon { width: 28px; height: 28px; border-radius: 6px; display: flex; align-items: center; justify-content: center; font-size: 13px; flex-shrink: 0; }
  ...
  .btn-item-del { background: transparent; border: none; color: var(--muted); cursor: pointer; padding: 4px; border-radius: 4px; font-size: 12px; transition: color 0.2s; line-height: 1; }
  .btn-item-del:hover { color: var(--danger); }

  .add-item-row { display: flex; gap: 8px; align-items: center; margin-top: 10px; padding-top: 12px; border-top: 1px dashed var(--border); flex-wrap: wrap; }
  .add-item-row input, .add-item-row select { background: var(--surface2); border: 1px solid var(--border); color: var(--text); padding: 7px 10px; border-radius: 7px; font-family: 'DM Mono', monospace; font-size: 11px; outline: none; transition: border-color 0.2s; }
  ...
  .btn-add-item { background: var(--surface2); border: 1px solid var(--accent); color: var(--accent); padding: 7px 12px; border-radius: 7px; font-family: 'Syne', sans-serif; font-weight: 700; font-size: 11px; cursor: pointer; transition: all 0.2s; white-space: nowrap; }
  .btn-add-item:hover { background: var(--accent); color: #0a0a0f; }

  /* Botão de browse para arquivos */
  .btn-browse { background: var(--surface2); border: 1px solid var(--border); color: var(--muted); padding: 7px 10px; border-radius: 7px; font-size: 11px; cursor: pointer; transition: all 0.2s; white-space: nowrap; }
  ...
  .modal { background: var(--surface); border: 1px solid var(--border); border-radius: 16px; padding: 32px; width: 100%; max-width: 480px; margin: 20px; animation: modalIn 0.2s ease; }
  ...
  .form-input { width: 100%; background: var(--surface2); border: 1px solid var(--border); color: var(--text); padding: 10px 12px; border-radius: 8px; font-family: 'Syne', sans-serif; font-size: 14px; outline: none; transition: border-color 0.2s; }
  .form-input:focus { border-color: var(--accent); }
  .color-options { display: flex; gap: 8px; flex-wrap: wrap; }
  .color-opt { width: 28px; height: 28px; border-radius: 50%; cursor: pointer; border: 2px solid transparent; transition: transform 0.15s, border-color 0.15s; }
  .color-opt:hover { transform: scale(1.15); }
  .color-opt.selected { border-color: white; transform: scale(1.1); }
  .modal-actions { display: flex; gap: 10px; justify-content: flex-end; margin-top: 24px; }
  .btn-cancel { background: transparent; border: 1px solid var(--border); color: var(--muted); padding: 9px 18px; border-radius: 8px; font-family: 'Syne', sans-serif; font-weight: 600; font-size: 13px; cursor: pointer; transition: all 0.2s; }
  .btn-cancel:hover { border-color: var(--text); color: var(--text); }
  .btn-save { background: linear-gradient(135deg, var(--accent), var(--accent2)); color: #0a0a0f; border: none; padding: 9px 20px; border-radius: 8px; font-family: 'Syne', sans-serif; font-weight: 700; font-size: 13px; cursor: pointer; transition: all 0.2s; }
  .btn-save:hover { opacity: 0.9; box-shadow: 0 4px 14px rgba(110,231,183,0.3); }

  .toast { position: fixed; bottom: 24px; right: 24px; background: var(--surface2); border: 1px solid var(--border); border-radius: 10px; padding: 12px 18px; font-size: 13px; display: flex; align-items: center; gap: 8px; z-index: 200; transform: translateY(80px); opacity: 0; transition: all 0.3s; max-width: 320px; }
...
<!-- Titlebar para arrastar a janela -->
<div class="titlebar">
  <span class="titlebar-title">🚀 &nbsp;Project Launcher</span>
</div>
...
<div class="app">
  <header class="header">
    <div class="header-top">
      ...
      <button class="btn-new" onclick="openModal()">＋ Novo Projeto</button>
    </div>
...
function renderProjects() {
  ...
  if (!filtered.length) {
    el.innerHTML = `<div class="empty-state"><div class="empty-icon">📂</div><h3>${q ? 'Nenhum projeto encontrado' : 'Nenhum projeto ainda'}</h3><p>${q ? 'Tente outro termo.' : 'Clique em "+ Novo Projeto" para começar.'}</p></div>`;
    return;
  }
  ...
  return `
    <div class="project-card ${isOpen ? 'open' : ''}" id="card-${p.id}">
      ...
      <div class="items-panel">
        ${items.length ? itemsHtml : '<p style="color:var(--muted);font-size:13px;padding:4px 0 8px;">Nenhum item ainda.</p>'}
        <div class="add-item-row">
          <input id="new-name-${p.id}" placeholder="Nome (opcional)">
          <input id="new-path-${p.id}" placeholder="Caminho ou URL...">
          <button class="btn-browse" onclick="browseFile('${p.id}')" title="Escolher arquivo">📁</button>
...
function openModal(id) {
  ...
  document.querySelectorAll('.color-opt').forEach((el, i) => el.classList.toggle('selected', COLORS[i] === selectedColor));
...
// Chama o backend para abrir diálogo de arquivo nativo
async function browseFile(pid) {
  const d = await api('POST', '/api/browse');
  if (d.path) {
    document.getElementById(`new-path-${pid}`).value = d.path;
    // Detecta se é exe/app
    const ext = d.path.split('.').pop().toLowerCase();
    if (['exe','bat','cmd','app','sh'].includes(ext)) {
      document.getElementById(`new-type-${pid}`).value = 'app';
    }
    // Nome automático
    const name = d.path.split(/[\\/]/).pop();
    if (!document.getElementById(`new-name-${pid}`).value) {
      document.getElementById(`new-name-${pid}`).value = name;
    }
  }
}
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if path == "/api/projects":
            proj = {"id": str(uuid.uuid4())[:8], "name": body.get("name", "Sem nome"), "desc": body.get("desc", ""), "color": body.get("color", "#6ee7b7"), "items": []}
            data["projects"].append(proj)
            save_data(data)
            self.send_json({"project": proj})

        elif path == "/api/browse":
            # Diálogo nativo de arquivo via tkinter
            try:
                ...
                file_path = filedialog.askopenfilename(
                    title="Selecionar arquivo ou programa",
                    filetypes=[("Todos", "*.*"), ("Executáveis", "*.exe"), ("Documentos", "*.pdf *.docx *.xlsx *.pptx"), ("Imagens", "*.png *.jpg")]
                )
                root.destroy()
                self.send_json({"path": file_path or ""})
            except Exception as e:
                self.send_json({"path": "", "error": str(e)})

        elif len(parts) == 4 and parts[3] == "items":
            pid = parts[2]
            for p in data["projects"]:
                if p["id"] == pid:
                    p.setdefault("items", []).append({"name": body.get("name", ""), "path": body.get("path", ""), "type": body.get("type", "file")})
                    save_data(data)
                    self.send_json({"items": p["items"]}); return
            ...
            for p in data["projects"]:
                if p["id"] == pid:
                    results = [{"ok": ok, "msg": msg, "item": item.get("name", "")} for item in p.get("items", []) for ok, msg in [open_item(item)]]
                    self.send_json({"results": results}); return
            self.send_json({"error": "not found"}, 404)
        else:
            self.send_response(404); self.end_headers()
    ...
    def do_PUT(self):
        ...
        if len(parts) == 3 and parts[1] == "projects":
            ...
            for p in data["projects"]:
                if p["id"] == pid:
                    p.update({"name": body.get("name", p["name"]), "desc": body.get("desc", p.get("desc", "")), "color": body.get("color", p.get("color", "#6ee7b7"))})
                    save_data(data)
                    self.send_json({"project": p}); return
        self.send_json({"error": "not found"}, 404)
    ...
    def do_DELETE(self):
        ...
        parts = path.strip("/").split("/")
        if len(parts) == 3 and parts[1] == "projects":
            data["projects"] = [p for p in data["projects"] if p["id"] != parts[2]]
            save_data(data)
...
if __name__ == "__main__":
    ...
    # Abre janela nativa com pywebview
    try:
        import webview
        ...
        print("pywebview não instalado. Rodando no browser...")
        import webbrowser
